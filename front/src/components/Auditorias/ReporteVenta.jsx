...
const Reportes = () => {
  ...
  const verTotalDrogueria = () => {
    if (fechaInicio && fechaFin) {
      ...
      axios
        .get(`${URL}reportes/verTotalDrogueria`, {
          params: {
            fechaInicio: inicio,
            fechaFin: fin,
          },
  ...
  const verTotalGastado = () => {
    if (fechaInicio && fechaFin) {
      ...
      const fin = format(fechaFin, "yyyy-MM-dd");
      axios
        .get(`${URL}reportes/verMontoTotalComprado`, {
          params: {
            fechaInicio: inicio,
            fechaFin: fin,
          },
  ...
  const verProductosMasComprados = () => {
    if (fechaInicio && fechaFin) {
      ...
      const fin = format(fechaFin, "yyyy-MM-dd");
      axios
        .get(`${URL}reportes/verTopProductosComprados`, {
          params: {
            fechaInicio: inicio,
            fechaFin: fin,
          },
  ...
  const verPromedioGastoPorCompra = () => {
    if (fechaInicio && fechaFin) {
      ...
      const fin = format(fechaFin, "yyyy-MM-dd");
      axios
        .get(`${URL}reportes/verPromedioGasto`, {
          params: {
            fechaInicio: inicio,
            fechaFin: fin,
          },
  ...
  return (
    <>
      ...
      <div style={{ marginLeft: "15px" }}>
        <p>
          MONTO TOTAL VENDIDO A LA FECHA:{" "}
          <strong>{formatCurrency(montototalComprado)}</strong>
        </p>
      </div>
            ...
            <BarChart data={totalDrogueria} layout="vertical">
              ...
              <XAxis
                ...
                domain={[
                  ...
                  totalDrogueria.length > 0
                    ? Math.max(
                        ...totalDrogueria.map((item) => item.total_comprado)
                      ) * 1.5
                    : 1000,
                ]}
            ...
            <BarChart data={productosmascomprados} layout="vertical">
              ...
              <XAxis
                ...
                domain={[
                  ...
                  productosmascomprados.length > 0
                    ? Math.max(
                        ...productosmascomprados.map(
                          (item) => item.total_cantidad
                        )
                      ) * 1.5
                    : 1000,
                ]}
              ...
              <Bar
                ...
                animationDuration={1500}
              >
                <LabelList dataKey="total_cantidad" position="right" />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        )}
